...
def main_worker_stage1(args, log_s1_name):
    ...
    print("==> Load dataset.")
    dataset_rgb_masked = get_data(args.dataset, args.data_dir, mode=args.mode, modal="rgb", suffix="_masked")
    dataset_rgb = get_data(args.dataset, args.data_dir, mode=args.mode, modal="rgb")
    dataset_ir = get_data(args.dataset, args.data_dir, mode=args.mode, modal="ir")
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            print('==> Create pseudo labels for unlabeled RGB data (masked)')
            # extract features of masked visible images
            cluster_loader_rgb_masked = get_test_loader(dataset_rgb_masked, args.height, args.width, 256, args.workers,
                                                        testset=sorted(dataset_rgb_masked.train))
            print('==> Extracting RGB data modality-specific features (masked)')
            features_rgb_masked, _ = extract_features(model, cluster_loader_rgb_masked, print_freq=50, mode=1)
            del cluster_loader_rgb_masked
            features_rgb_masked = torch.cat(
                [features_rgb_masked[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb_masked.train)], 0)
            ...
            rerank_dist_rgb = compute_jaccard_distance(features_rgb_masked, k1=args.k1, k2=args.k2, search_option=3)
        ...
        if epoch >= 0 and ((epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
            ...
            for trial in range(1):
                ...
                trial_gall_loader = data.DataLoader(trial_gallset, batch_size=64, shuffle=False,
    ...
    for trial in range(10):
        ...
        trial_gall_loader = data.DataLoader(trial_gallset, batch_size=64, shuffle=False, num_workers=args.workers)
...
def main_worker_stage2(args, log_s1_name, log_s2_name):
    ...
    checkpoint = load_checkpoint('../Code/usvi_masked/logs72e_sysumm01/stage112/model_best.pth.tar')
    ...
    for epoch in range(args.epochs):
        ...
        dist_cm = torch.matmul(features_rgb, features_ir.t()).numpy()  # compute similarity matrix (22258,11909)
...
！
        ...
        @torch.no_grad()
        def generate_cluster_features_hat(cluster_feature, features1, labels1, features2,
                                          labels2):

...
def intdict2ndarray(d, default_val=-1):
    lenth = max(d.keys())
    print(lenth + 1)
    arr = np.zeros(lenth + 1) + default_val
...
class knn_faiss():
    ...
    def __init__(self, feats, k, knn_method='faiss-cpu', verbose=True):
        ...
        with Timer('[{}] query topk {}'.format(knn_method, k), verbose):
            ...
            self.knns = [(np.array(nbr, dtype=np.int32), 1 - np.array(sim, dtype=np.float32)) for nbr, sim in
                         zip(nbrs, sims)]
...
def cluster_by_infomap(nbrs, dists, min_sim, cluster_num=2):
    ...
    for (i, j), sim in tqdm(links.items()):
        ...
        _ = infomapWrapper.addLink(int(i), int(j))
    # 聚类运算
    infomapWrapper.run()
    ...
    for node in infomapWrapper.tree.leafIter():
        ...
        label2idx[node.moduleIndex()].append(node.physIndex)

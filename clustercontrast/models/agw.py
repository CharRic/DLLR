    ...
    def __init__(self, in_channels, reduc_ratio=2):
        ...
        self.W = nn.Sequential(
            nn.Conv2d(in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1, stride=1,
                      padding=0), nn.BatchNorm2d(self.in_channels), )
...
class visible_module(nn.Module):
    def __init__(self, arch='resnet50'):
        ...
        model_v = resnet50_agw(pretrained=True, last_conv_stride=1, last_conv_dilation=1)
...
class thermal_module(nn.Module):
    def __init__(self, arch='resnet50'):
        ...
        model_t = resnet50_agw(pretrained=True, last_conv_stride=1, last_conv_dilation=1)
        # avg pooling to global pooling
    ...
    def forward(self, x1, x2, modal=0, label_1=None, label_2=None):
        ...
        if self.training:
            return feat, feat[:single_size], feat[single_size:], label_1, label_2, x_pool[:single_size], x_pool[
                                                                                                         single_size:]  # x_pool#, self.classifier(feat)
        else:
            # return self.l2norm(x_pool), self.l2norm(feat)
            return self.l2norm(feat)  # self.l2norm(x_pool)#,

        # if self.training:  #     return x_pool, self.classifier(feat)  # else:  #     return self.l2norm(x_pool), self.l2norm(feat)

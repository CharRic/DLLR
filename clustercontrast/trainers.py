    ...
    def train(self, epoch, data_loader_rgb=None, data_loader_ir=None, data_loader_rgb2ir=None, data_loader_ir2rgb=None,
        ...
        for i in range(train_iters):
            ...
            if stage == 1:
                ...
                dis_label = torch.cat((torch.ones(inputs_rgb.size(0)), torch.zeros(inputs_ir.size(0))),
                                      dim=0).cuda()
            ...
            if (i + 1) % print_freq == 0:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      'Data {:.3f} ({:.3f})\t'
                      'Loss {:.3f} ({:.3f})\t'
                      'Loss rgb {:.3f}\tLoss ir {:.3f}\t'
                      .format(epoch, i + 1, train_iters, batch_time.val, batch_time.avg, data_time.val, data_time.avg,
